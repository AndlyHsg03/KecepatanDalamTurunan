function hitung() {
    ...
    try {
        ...
        document.getElementById("kecepatan").innerHTML = `
            ...
            <ul>
                <li>Persamaan posisi kendaraan: <em>${persamaan.replace(/\*\*/g, "^").replace(/\*/g, "")}</em></li>
                <li>Turunan posisi kendaraan (kecepatan): <em>${turunanPersamaan.replace(/\*\*/g, "^").replace(/\*/g, "")}</em></li>
                <li>Substitusi t = ${t} ke turunan posisi kendaraan: <em>${turunanPersamaan.replace(/t/g, t)}</em></li>
            ...
            <ul>
                <li>1 meter = 1/1000 kilometer (m ke km)</li>
                <li>1 detik = 1/3600 jam (s ke jam)</li>
...
function hitungTurunan(persamaan) {
    // Pisahkan setiap suku berdasarkan operator +
    const suku = persamaan.split("+").map(s => s.trim());
    let turunan = [];

    suku.forEach((s) => {
        // Deteksi bentuk suku seperti 2*t^2
        ...
        if (match) {
            ...
            if (pangkat > 1) {
                // Turunan untuk suku dengan pangkat > 1
            ...
            // Suku konstan diabaikan
        } else {
            // Tangani kasus tidak terduga
            turunan.push(s);
        }
    });
